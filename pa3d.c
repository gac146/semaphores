...
void InitRoad ()
{
	...
	Regshm((char *) &road, sizeof(road));

	for(int i=0; i <= MAXPROCS + 10; i++) {
		road.sems[i] = Seminit(1);
	}

	road.busy = FALSE;
	road.mySemSideOne = Seminit(0);
	road.mySemSideTwo = Seminit(0);
	road.mutex = Seminit(1);
	road.firstPosition = Seminit(0);
	road.countSideOne = 0;
	road.countSideTwo = 0;
	road.count = 0;
	road.waiting = 0;
	road.switchSides = EAST;
	road.start = TRUE;
	road.currDir = -1;
}
	...
	int mph;			// speed of car
{
	...
	// Determines the value of p and np
	if(from == WEST){ 
		p = 0;
		np = 1;
	}
	...
	// Checking if there is a car in the first postion
	if(road.currDir == WEST || road.currDir == EAST) {
		if(from == road.currDir) {
			Wait(road.firstPosition);
		}
	}

	// if the road is busy these cars have to wait until road is clear
	if(road.busy) {
		if(from != road.currDir && road.count > 0) {
			road.waiting++;
			
			if(road.start) {
				road.switchSides = from;
				road.start = FALSE;
			}
			
			if(from == WEST) {
				road.countSideOne++;
				Wait(road.mySemSideOne);
			} else {
				road.countSideTwo++;
				Wait(road.mySemSideTwo);
			}
			road.switchSides = (from == WEST) ? EAST : WEST;
		} else if(road.waiting > 0) {
			road.waiting++;
	...
	for (i = 1; i < NUMPOS; i++) {
		if (from == WEST) {
			p = i;
			np = i + 1;
			if(p > 1) Signal(road.firstPosition);
		} else {
			p = NUMPOS + 1 - i;
			np = p - 1;
			if(p < NUMPOS) Signal(road.firstPosition);
		}
	...
	if(from == WEST) Signal(road.sems[p+1]);
	else Signal(road.sems[p-1]);
	...
	if(road.count == 0){
		...
		if(road.waiting > 0){
			road.busy = TRUE;
			int tmp = road.waiting;
			for(int i=0; i < tmp; i++) {
				if(road.countSideOne == 0 || road.countSideTwo == 0) {
					road.waiting--;
					road.start = FALSE;
					if(road.countSideOne > 0) {
						road.countSideOne--;
						Signal(road.mySemSideOne);
					} else {
						road.countSideTwo--;
						Signal(road.mySemSideTwo);
					}
				} else {
					if(road.switchSides == WEST) {
						road.countSideOne--;
						Signal(road.mySemSideOne);
					} else {
						road.countSideTwo--;
